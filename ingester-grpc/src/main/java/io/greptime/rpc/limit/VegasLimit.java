...
public class VegasLimit extends AbstractLimit {
    ...
    protected int _update(long startTime, long rtt, int inflight, boolean didDrop) {
        Ensures.ensure(rtt > 0, "rtt must be > 0 but got " + rtt);
    ...
    private int updateEstimatedLimit(long rtt, int inflight, boolean didDrop) {
        ...
        int queueSize = (int) Math.ceil(currLimit * (1 - (double) this.rtt_noload / rtt));
        ...
        if (this.logOnLimitChange && (int) newLimit != (int) currLimit) {
            LOG.info(
                    "New limit={}, previous limit={}, minRtt={} ms, winRtt={} ms, queueSize={}.",
                    (int) newLimit,
                    (int) currLimit,
